...
@interface UIColor (UIColorExtensionsPrivate)
...
{
    return ([self rgbHex] == [color rgbHex]);
}
...
- (UIColor*) colorByBlendingWithColor:(UIColor *)color alpha:(CGFloat)alpha {
    ...
    [self red:&ar green:&ag blue:&ab alpha:&aa];
    [color red:&br green:&bg blue:&bb alpha:&ba];
    
    cr = ar * alpha + br * (1.0 - alpha);
    cg = ag * alpha + bg * (1.0 - alpha);
    cb = ab * alpha + bb * (1.0 - alpha);
    
    return [UIColor colorWithRed:cr green:cg blue:cb alpha:1.0];
}
...
+(UIColor*) highlightedColor:(UInt32) hex {
    return [[UIColor colorWithRGBHex:hex] colorByMultiplyingBy:0.8];
}

+(UIColor*) highlightedColorFromColor:(UIColor*)color {
    return [color colorByMultiplyingBy:0.8];
}
